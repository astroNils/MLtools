...
def get_boulder_dicts(json_file, img_dir, dataset_type):
    ...
    df_copy = df.copy()
    df_copy["file_name"] = img_dir / df.file_name
...
def generate_custom_dataset(json_file, img_dir, dataset_name):
    for d in ["train", "test"]:
        ...
        MetadataCatalog.get(dataset_name + "_" + d).set(thing_classes=["sterile", "fertile"])
    pollen_metadata = MetadataCatalog.get(dataset_name + "_train")
    return pollen_metadata
